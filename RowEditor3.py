...
gsheetCSVURL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vSbqvvU4R0NH1K3hT3pqq0H-ENlFuf-rrb-LPo7lSejZ7BHSxQaAw6YBm29b2jXAAySfE4pJS2UVSl5/pub?output=csv'
"""
#APRIL 28 TESTING
...
def downLoadgSheet(gSheetURL=gsheetCSVURL, CSVName=CSVname):  ########NEWVERSION.  Presumes the CSV is published at gsheetCSVURL
    print(f'Downloading {gSheetURL}', end=' ')
    ret = requests.get(gSheetURL)
    print(ret)
    f=open(CSVname,'w')
    f.write(ret.text)
    f.close()
    print(f'created {CSVname}')
    return CSVname


def upLoadCSV(CSVname = CSVname):
...
def downUp(e):
    print(str(e))
    downLoadgSheet()
    recs = recsFromCSV(CSVname)
    
    for i, rec in enumerate(recs): #renumber Freshness 
        rec['Freshness'] = i+1
        recs[i] = rec
    
    print('renumbered Freshness last record =', i)
    recsToCSV(recs, CSVname)
...
def improveRegenCanada():
    text = T.value
    text = '\n'.join(T.value.split('\n')[1:])
    import re
    new_text = re.sub(r'(?m)^(?=[A-Z\s]+$)', '\n', text)
    T.value = new_text.replace('\n\n\n','\n\n').replace('acres','acres\n\n',1)


# In[17]:


def improveRegenEarth():
    #for i in range(140):
        T.value = T.value.replace('<br><br>','\n\n')
        doStore('test')
        inc('test')
        sleep(2)
...
def improveGER():
    ...
    twitchers['Picture'].value = twitchers['Thumbnails'].value
    Test = T.value
    Test  = Test.replace('<p>','').replace('</p>','\n')
    Test = Test.replace('div','').replace('</div>','\n\n')
    Test = Test.replace('<br>','').replace('<br/>','\n\n')
    Test = Test.replace('figure','').replace('</figure>','\n\n')
    ...
    Test = Test.replace('<li>\n','\n<li>')
    Test = Test.replace('>\n','\>')
    Test= re.sub(r'\s+</a>', '</a>', Test)
    Test=Test.replace('Video ','\n\nVideo ' )
    Test=Test.replace('\n:',':' )
    Test=Test.replace(':\n',':' )
    Test=Test.replace('<hr/>','\n\n<hr/>')
    Test=Test.replace('\n<em>','<em>').replace('\n</em>','')
    Test=Test.replace('<em>','').replace('</em>','')
    Test=Test.replace('<h4>','').replace('</h4>','\n')
    Test=Test.replace('<body>','')
    T.value = Test
    print('done')
#improveGER()    
...
#login manually using email and password
...
if 0:
    ...
    if img:
        img.decompose()
    try:
        if soup.h4.text == soup.h5.text:
            soup.h5.decompose()
    except AttributeError:
        pass
    text = str(soup.body).replace('<body>','').replace('</i>','').replace('<i>','')
    text = text.replace('<br/><br/>', '\n\n').replace('<br/>','\n')
    text=text.replace('</p><hr/>','\n<i>')
    text=text.replace('<h4>','\n\n').replace('<h5>',''.replace('<h6>',''))
    text = text.replace('<h6>','')
    text=text.replace('</h4>','\n').replace('</h5>','\n'.replace('</h6>',''))
    text=text.replace('</h6></body>','')
    text=text.replace('<hr/>','<hr/><p>')
    T.value = text[2:]
    ImageSourcesClick('test')
    sleep(1)
    ImageSourcesClick('test')
    T.value.replace('\n\n\n','\n\n')
    T.value.replace('\n\n\n','\n\n')
...
if 0:
    if not twitchers['Location'].value:
        twitchers['Location'].value = twitchers['Project Name'].value.split(':')[0]
#doStore('test')
#inc('test')
...
if 0:
    orig = """"<i><a href='https://www.uniteddesigners.org/deeenzaamheidfarm-thenetherlands'>This site is is developed with high regard to Graphic work.  See it in full glory <b>here</b></a>.
    </i>"""

    wanted = f"""<i><a href='{twitchers['Source'].value}'>This site is is developed with high regard to Graphic work.  See it in full glory <b>here</b></a>
    </i>

    """

    if T.value.startswith(orig):
        T.value = T.value.replace(orig, wanted)
    else:
        T.value = wanted + T.value 
    doStore('test')
    sleep(1)
    inc('test')
